...
var BenchmarkJS = function() {
	...
	this.haxe3Data = null;
	this.haxe4Data = null;
	this.haxeNightlyData = null;
	this.haxe3Version = "3";
	this.haxe4Version = "4";
	this.haxeNightlyVersion = "nightly";
	...
	this.withHaxe3 = true;
	this.withHaxe4 = true;
	this.withHaxeNightly = true;
	this.last20Days = true;
	...
	$("#withHaxe3").change($bind(this,this.changeWithHaxe3));
	$("#withHaxe4").change($bind(this,this.changeWithHaxe4));
	$("#withHaxeNightly").change($bind(this,this.changeWithHaxeNightly));
	$("#last20Days").change($bind(this,this.changeLast20Days));
};
...
BenchmarkJS.prototype = {
	...
	,changeWithHaxe3: function(event) {
		this.withHaxe3 = $("#withHaxe3").is(":checked");
		this.showData();
	}
	,changeWithHaxe4: function(event) {
		this.withHaxe4 = $("#withHaxe4").is(":checked");
		this.showData();
	}
	,changeWithHaxeNightly: function(event) {
		this.withHaxeNightly = $("#withHaxeNightly").is(":checked");
		this.showData();
	}
	,changeLast20Days: function(event) {
		this.last20Days = $("#last20Days").is(":checked");
		this.showData();
	}
	...
	,requestArchivedData: function() {
		...
		request.onError = function(msg) {
			console.log("srcPages/BenchmarkJS.hx:153:","failed to download Haxe 3 data: " + msg);
		};
		request.request();
		...
		request1.onError = function(msg1) {
			console.log("srcPages/BenchmarkJS.hx:164:","failed to download Haxe 4 data: " + msg1);
		};
		request1.request();
		var request2 = new haxe_http_HttpJs("data/archiveHaxeNightly.json");
		request2.onData = function(data2) {
			var parser2 = new JsonParser_$1();
			_gthis.haxeNightlyData = parser2.fromJson(data2,"archiveHaxeNightly.json");
			_gthis.checkLoaded();
		};
		request2.onError = function(msg2) {
			console.log("srcPages/BenchmarkJS.hx:175:","failed to download Haxe 3 data: " + msg2);
		};
		request2.request();
	}
	...
	,showData: function() {
		this.haxe3Version = this.haxe3Data[this.haxe3Data.length - 1].haxeVersion;
		this.haxe4Version = this.haxe4Data[this.haxe4Data.length - 1].haxeVersion;
		this.haxeNightlyVersion = this.haxeNightlyData[this.haxeNightlyData.length - 1].haxeVersion;
		this.showLatest();
		...
		this.showHistory("NodeJS (ES6)","nodeES6Benchmarks");
		this.showHistory("Neko","nekoBenchmarks");
		this.showHistory("eval","evalBenchmarks");
	...
	,showLatest: function() {
		...
		var latestHaxeNightlyData = this.haxeNightlyData[this.haxeNightlyData.length - 1];
		var labels = ["C++","C#","eval","Hashlink","Hashlink/C","Java","JVM","Neko","NodeJS","NodeJS (ES6)","PHP","Python"];
		...
		var latestHaxeNightlyData1 = latestHaxeNightlyData.haxeVersion;
		var _g4 = [];
		var _g5 = 0;
		...
		var haxeNightlyDataset = { label : latestHaxeNightlyData1, backgroundColor : "#66FF66", borderColor : "#33FF33", borderWidth : 1, data : _g4};
		var data = { labels : labels, datasets : [haxe3Dataset,haxe4Dataset,haxeNightlyDataset]};
		var _g6 = 0;
		...
		while(_g6 < _g7.length) {
			...
			switch(target.name) {
			case "JVM":case "NodeJS (ES6)":case "eval":
				continue;
			default:
			}
		...
		var _g10 = 0;
		var _g11 = latestHaxeNightlyData.targets;
		while(_g10 < _g11.length) {
			var target2 = _g11[_g10];
			++_g10;
			var index2 = data.labels.indexOf(target2.name);
			if(index2 < 0) {
				continue;
			}
			haxeNightlyDataset.data[index2] = target2.time;
		}
		var options = { type : "bar", data : data, options : { responsive : true, animation : { duration : 0}, legend : { position : "top"}, title : { display : true, text : "latest benchmark results"}, tooltips : { mode : "index", intersect : false}, hover : { mode : "nearest", intersect : true}, scales : { yAxes : [{ scaleLabel : { display : true, labelString : "runtime in seconds"}}]}}};
		var _this = this.chartObjects;
		if(!(__map_reserved["latest"] != null ? _this.existsReserved("latest") : _this.h.hasOwnProperty("latest"))) {
			...
			if(__map_reserved["latest"] != null) {
				_this1.setReserved("latest",chart);
			} else {
				_this1.h["latest"] = chart;
			}
			return;
		}
	...
	,showHistory: function(target,canvasId) {
		var _gthis = this;
		var graphDataSets = this.makeGraphDatasets(target);
		var _g = [];
		var _g1 = 0;
		var _g2 = graphDataSets;
		while(_g1 < _g2.length) {
			var v = _g2[_g1];
			++_g1;
			if((function(info) {
				if(info.type == data_DatasetType.Haxe3 && (target == "JVM" || target == "eval" || target == "NodeJS (ES6)")) {
					return false;
				}
				switch(_gthis.showAverage._hx_index) {
				case 0:
		...
		var datasetData = [];
		if(this.withHaxe3) {
			datasetData = datasetData.concat(this.collectRunData(target,this.haxe3Data,data_DatasetType.Haxe3));
		}
		if(this.withHaxe4) {
			datasetData = datasetData.concat(this.collectRunData(target,this.haxe4Data,data_DatasetType.Haxe4));
		}
		if(this.withHaxeNightly) {
			datasetData = datasetData.concat(this.collectRunData(target,this.haxeNightlyData,data_DatasetType.HaxeNightly));
		}
		datasetData.sort($bind(this,this.sortDate));
		datasetData = this.mergeTimes(datasetData);
		var now = new Date().getTime();
		now -= 1728000;
		var _g3 = 0;
		while(_g3 < datasetData.length) {
			var item = datasetData[_g3];
			++_g3;
			if(!this.showDate(item.date,now)) {
				continue;
			}
			data1.labels.push(item.date);
			var _g4 = 0;
			while(_g4 < graphDataSets.length) {
				var graph = graphDataSets[_g4];
				++_g4;
		...
		var result = new Array(graphDataSets.length);
		var _g5 = 0;
		var _g11 = graphDataSets.length;
		while(_g5 < _g11) {
			var i = _g5++;
			result[i] = graphDataSets[i].dataset;
		}
		data1.datasets = result;
		...
		var _this = this.chartObjects;
		if(!(__map_reserved[target] != null ? _this.existsReserved(target) : _this.h.hasOwnProperty(target))) {
		...
		var _this2 = this.chartObjects;
		var chart1 = __map_reserved[target] != null ? _this2.getReserved(target) : _this2.h[target];
		chart1.data = data1;
		chart1.update();
	}
	,showDate: function(dateVal,onlyAfter) {
		if(!this.last20Days) {
			return true;
		}
		var date = HxOverrides.strDate(dateVal);
		var time = date.getTime();
		return time > onlyAfter;
	}
	,mergeTimes: function(datasetData) {
		var result = [];
		var lastDataPoint = null;
		var _g = 0;
		while(_g < datasetData.length) {
			var data = datasetData[_g];
			++_g;
			...
			if(HxOverrides.substr(lastDataPoint.date,0,lastDataPoint.date.length - 2) != HxOverrides.substr(data.date,0,data.date.length - 2)) {
				lastDataPoint = data;
				result.push(data);
				continue;
			}
			var _g1 = new haxe_iterators_MapKeyValueIterator(data.time);
			while(_g1.hasNext()) {
				var _g11 = _g1.next();
				var key = _g11.key;
				var val = _g11.value;
			...
			var _g2 = new haxe_iterators_MapKeyValueIterator(data.sma);
			while(_g2.hasNext()) {
				var _g12 = _g2.next();
				var key1 = _g12.key;
				var val1 = _g12.value;
				lastDataPoint.sma.set(key1,val1);
			}
		}
		return result;
	}
	,collectRunData: function(target,resultsData,type) {
		var average = this.averageFactory(this.windowSize);
		var datasetData = [];
		var _g = 0;
		while(_g < resultsData.length) {
			var run = resultsData[_g];
			++_g;
			...
			_g1.set(type,data__$TestRun_TimeValue_$Impl_$.fromFloat(time));
			var _g11 = new haxe_ds_EnumValueMap();
			_g11.set(type,average.getAverage());
			datasetData.push({ time : _g1, sma : _g11, date : run.date});
		}
		return datasetData;
	}
	,makeGraphDatasets: function(target) {
		return [this.makeGraphDataset(data_DatasetType.Haxe3,false,target + " (Haxe 3)","#FF0000","#FF0000"),this.makeGraphDataset(data_DatasetType.Haxe3,true,target + " (Haxe 3 avg)","#FFCCCC","#FFCCCC"),this.makeGraphDataset(data_DatasetType.Haxe4,false,target + " (Haxe 4)","#0000FF","#0000FF"),this.makeGraphDataset(data_DatasetType.Haxe4,true,target + " (Haxe 4 avg)","#CCCCFF","#CCCCFF"),this.makeGraphDataset(data_DatasetType.HaxeNightly,false,target + " (Haxe nightly)","#66FF66","#66FF66"),this.makeGraphDataset(data_DatasetType.HaxeNightly,true,target + " (Haxe nightly avg)","#CCFFCC","#CCFFCC")];
	}
	,makeGraphDataset: function(type,movingAverage,label,borderColor,backgroundColor) {
		return { type : type, movingAverage : movingAverage, dataset : { label : label, backgroundColor : backgroundColor, borderColor : borderColor, borderWidth : 1, fill : false, spanGaps : true, data : []}};
	}
...
var data_DatasetType = $hxEnums["data.DatasetType"] = { __ename__ : true, __constructs__ : ["Haxe3","Haxe4","HaxeNightly"]
	,Haxe3: {_hx_index:0,__enum__:"data.DatasetType",toString:$estr}
	,Haxe4: {_hx_index:1,__enum__:"data.DatasetType",toString:$estr}
	,HaxeNightly: {_hx_index:2,__enum__:"data.DatasetType",toString:$estr}
};
